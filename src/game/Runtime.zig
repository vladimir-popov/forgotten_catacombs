...
pub fn openFile(self: Runtime, file_path: [:0]const u8, mode: FileMode, buffer: []u8) anyerror!OpaqueFile {
...
pub fn isFileExists(self: Runtime, path: [:0]const u8) anyerror!bool {
    return try self.vtable.isFileExists(self.context, path);
}

pub fn deleteFileIfExists(self: Runtime, path: [:0]const u8) anyerror!void {
