...
pub fn ComponentsIterator(comptime Components: type) type {
    return struct {
            ...
            return struct {
                ...
                pub fn next(self: *@This()) ?struct { Entity, *C1, *C2, *C3, *C4 } {
                    while (self.idx < self.parent.entities.len) {
                        const entity = self.parent.entities[self.idx];
                        self.idx += 1;
                        if (self.parent.manager.get4(entity, C1, C2, C3, C4)) |res| {
                            return .{ entity, res[0], res[1], res[2], res[3] };
                        }
                    }
                    return null;
                }
        ...
        pub fn of4(self: *const Self, comptime C1: type, C2: type, C3: type, C4: type) Iterator4(C1, C2, C3, C4) {
            return .{ .parent = self };
        }
    };
}
