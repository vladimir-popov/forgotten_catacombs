...
pub fn initNew(
    ...
    const weapon = try self.registry.addNewEntity(g.entities.presets.Weapons.get(.pickaxe));
    const light = try self.registry.addNewEntity(g.entities.presets.Items.get(.torch));
