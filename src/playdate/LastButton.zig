...
inline fn isReleased(self: Self) bool {
    return self.pressed_at == 0;
}

pub fn pop(self: *Self, now_ms: u64) ?g.Button {
    const game_button = g.Button.GameButton.fromCode(self.buttons) orelse return null;
...
pub fn handleEvent(
    ...
    const runtime: *PlaydateRuntime = @alignCast(@fieldParentPtr("last_button", self));
    if (runtime.is_menu_shown) {
        log.debug("Menu was shown. Ignore buttons {d}.", .{buttons});
        return 0;
    }
    if (down > 0) {
