...
pub fn generate(csv: []const u8, writer: *std.Io.Writer) !void {
    ...
    const weight_column = findColumn(header_fields[0..header_count], "RND Weight") orelse
        return error.MissingWeightColumn;
    ...
    while (lines.next()) |raw_line| {
        ...
        if (field_count <= @max(weight_column, name_column))
            return error.MissingField;

        const name = fields[name_column];
        const weight = fields[weight_column];
        _ = std.fmt.parseInt(u8, weight, 10) catch return error.InvalidWeight;
