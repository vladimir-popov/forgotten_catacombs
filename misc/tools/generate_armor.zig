...
fn generate(csv: []const u8, writer: *std.Io.Writer, allocator: std.mem.Allocator) !void {
    ...
    const armor_column = findColumn(header_fields[0..header_count], "Armor") orelse
        return error.MissingArmorColumn;
    const price_column = findColumn(header_fields[0..header_count], "Price") orelse
        return error.MissingPriceColumn;

    var rows: std.ArrayList(ArmorRow) = .empty;
    ...
    while (lines.next()) |raw_line| {
        ...
        const columns = [_]usize{ name_column, armor_column, price_column };
        ...
        const price = std.fmt.parseInt(u16, fields[price_column], 10) catch
            return error.InvalidPrice;
        try rows.append(allocator, .{
            .name = fields[name_column],
            .protection = try parseProtection(fields[armor_column]),
            .price = price,
        });
    }

    std.mem.sort(ArmorRow, rows.items, {}, lessThanByName);
    ...
    for (rows.items) |row| {
        ...
        try writer.writeAll(": c.Components = archetype.armor(.{\n");
        try writer.print(
            "    .armor = .{{ .protection = .range({d}, {d}) }},\n",
            .{ row.protection.min, row.protection.max },
        );
        try writer.writeAll("    .description = .{ .preset = .");
        try writeSnakeCase(writer, row.name);
        try writer.writeAll(" },\n    .sprite = .{ .codepoint = cp.armor },\n");
        try writer.print("    .price = .{{ .value = {d} }},\n", .{row.price});
        try writer.writeAll("}),\n");
    }
}

fn parseProtection(value: []const u8) !Protection {
    const ascii_separator = std.mem.indexOfScalar(u8, value, '-');
    const unicode_separator = std.mem.indexOf(u8, value, "–");
    const separator = ascii_separator orelse unicode_separator orelse
        return error.InvalidArmor;
    const separator_length: usize = if (ascii_separator != null) 1 else "–".len;

    const min = std.fmt.parseInt(u8, value[0..separator], 10) catch
        return error.InvalidArmor;
    const max = std.fmt.parseInt(u8, value[separator + separator_length ..], 10) catch
        return error.InvalidArmor;
    if (min > max)
        return error.InvalidArmor;
...
fn lessThanByName(_: void, lhs: ArmorRow, rhs: ArmorRow) bool {
